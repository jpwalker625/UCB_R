...
cal.mapping <- usa.counties.mapping[usa.counties.mapping@data$STATE == '06', ]
...
employment.stats <- rename(employment.stats, county = area_title)
...
#map the data
range(employment.stats$unemployed_rate)

bins <- c(0, 2.7, 6, 9, 12, 15, 18, Inf)

pal <- colorBin(palette = "BuPu", domain = employment.stats$unemployed_rate, bins = bins)

labels <- sprintf("<strong>%s</strong><br/>%g%%", employment.stats$county, employment.stats$unemployed_rate) %>%
...
counties.map <- leaflet(cal.mapping) %>%
  ...
  addPolygons(fillColor = ~pal(employment.stats$unemployed_rate),
              weight = 2,
              opacity = 1,
              color = "white",
              dashArray = "3",
  ...
  addLegend(position = 'topright', 
            pal = pal, 
            values = ~employment.stats$unemployed_rate,
